...
import { 
  ...
  Calendar, Layers, Zap, ExternalLink, QrCode, LogOut, ChevronRight, Check
} from 'lucide-react';
...
export default function AccountPage() {
  ...
  // State cho việc mở lại modal thanh toán cho đơn PENDING
  ...
  const loadAccountData = async () => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        setUserData({ credits: 0, role: 'user' });
      }
      ...
      try {
        ...
        const list = orderSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#2D3E3A] font-sans relative pb-20">
      ...
      <nav className="border-b border-[#E2E8E5] bg-[#FFFFFF]/90 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-18 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={logout}
              className="px-3.5 py-2 rounded-2xl bg-[#FAF8F5] hover:bg-red-50 hover:text-red-700 border border-[#E2E8E5] text-[#5F736E] text-xs sm:text-sm font-medium transition-all flex items-center gap-1.5 cursor-pointer"
            >
              <LogOut size={15} />
      ...
      {/* MAIN CONTAINER */}
      <main className="max-w-6xl mx-auto px-4 pt-8 space-y-8 relative z-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              {user?.photoURL ? (
                ...
                <img src={user.photoURL} alt="Avatar" className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl border-2 border-[#267D71]/40 object-cover shadow-sm" />
              ) : (
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-[#FFEFB3] text-[#013E37] flex items-center justify-center text-2xl font-bold border border-[#F9E79F]">
                  {user?.displayName?.[0] || 'U'}
                </div>
              )}
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h1 className="text-xl sm:text-2xl font-bold font-heading text-[#0D2B26]">{user?.displayName || 'Khách hàng'}</h1>
                  {userData?.role === 'coach' && (
                ...
                <p className="text-xs text-[#5F736E] font-mono">{user?.email}</p>
                <div className="flex items-center gap-2 pt-1 text-xs text-[#267D71] font-semibold">
                  <ShieldCheck size={14} />
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-[#FAF8F5] border border-[#E2E8E5] p-4 rounded-2xl text-center space-y-1 min-w-[130px]">
                <div className="text-[11px] text-[#5F736E] uppercase font-bold tracking-wider">Số Lượt Báo Cáo</div>
                <div className="text-2xl sm:text-3xl font-extrabold text-[#013E37] font-mono">{creditsCount}</div>
                <div className="text-[10px] text-[#267D71] font-semibold">Lượt có sẵn</div>
              </div>

              <div className="bg-[#FAF8F5] border border-[#E2E8E5] p-4 rounded-2xl text-center space-y-1 min-w-[130px]">
                <div className="text-[11px] text-[#5F736E] uppercase font-bold tracking-wider">Gói Hội Viên</div>
                <div className="text-sm sm:text-base font-extrabold text-[#0D2B26] truncate">
                  {isSubscriptionActive ? (subscription?.planName || 'Coach VIP') : 'Gói Tiêu Chuẩn'}
                </div>
                <div className="text-[10px] text-[#5F736E]">
                  {isSubscriptionActive ? `HSD: ${new Date(subscription.expiresAt).toLocaleDateString('vi-VN')}` : 'Vĩnh viễn'}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* TABS NAVIGATION */}
        <div className="flex gap-2 border-b border-[#E2E8E5] pb-2">
          <button
        ...
        {/* TAB 1: TỔNG QUAN & GÓI ĐANG DÙNG */}
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              {/* Thẻ Quản lý Lượt Báo Cáo */}
              <div className="card-surface rounded-3xl p-6 sm:p-7 space-y-5 border border-[#E2E8E5] shadow-sm flex flex-col justify-between">
                <div className="space-y-3">
                  <div className="w-12 h-12 rounded-2xl bg-[#EEF5F3] text-[#267D71] flex items-center justify-center border border-[#267D71]/20">
                    <Sparkles size={24} />
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-[#FAF8F5] border border-[#E2E8E5] flex items-center justify-between">
                    <span className="text-xs font-semibold text-[#5F736E]">Lượt hiện tại:</span>
                    <span className="font-mono text-2xl font-extrabold text-[#013E37]">{creditsCount} bài</span>
                  </div>
                </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row gap-3">
                  <a
                    href="/"
                    className="flex-1 py-3.5 px-4 rounded-2xl btn-primary text-xs font-bold flex items-center justify-center gap-2 shadow-sm text-center"
                  >
                  ...
                  <a
                    href="/pricing"
                    className="py-3.5 px-5 rounded-2xl bg-[#EEF5F3] hover:bg-[#E2EFEA] text-[#013E37] text-xs font-bold border border-[#267D71]/30 transition-all text-center"
                  >
                    Nạp Thêm Lượt
                  </a>
                </div>
              ...
              {/* Thẻ Quyền Lợi & Hạn Gói */}
              <div className="card-surface rounded-3xl p-6 sm:p-7 space-y-5 border border-[#E2E8E5] shadow-sm flex flex-col justify-between">
                <div className="space-y-3">
                  <div className="w-12 h-12 rounded-2xl bg-[#FFEFB3] text-[#013E37] flex items-center justify-center border border-[#F9E79F]">
                    <Zap size={24} />
                  </div>
                  <h3 className="text-xl font-bold font-heading text-[#0D2B26]">Gói Quyền Lợi Hệ Thống</h3>
                  <p className="text-xs text-[#5F736E] leading-relaxed">
                    {isSubscriptionActive 
                      ? 'Bạn đang sử dụng gói hội viên cao cấp với đầy đủ tính năng White-label thương hiệu riêng và công cụ CRM thông minh.'
                      : 'Bạn đang ở gói dịch vụ tiêu chuẩn. Nâng cấp lên gói Chuyên gia để hưởng chiết khấu nạp sỉ chỉ từ 25k-49k/bài.'}
                  </p>

                  <div className="p-4 rounded-2xl bg-[#FAF8F5] border border-[#E2E8E5] space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-[#5F736E]">Loại tài khoản:</span>
                      <span className="font-bold text-[#0D2B26]">{isSubscriptionActive ? (subscription?.planName || 'Life Coach VIP') : 'Khách hàng cá nhân'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#5F736E]">Thời hạn sử dụng:</span>
                      <span className="font-bold text-[#267D71]">{isSubscriptionActive ? `${new Date(subscription.expiresAt).toLocaleDateString('vi-VN')}` : 'Vĩnh viễn'}</span>
                    </div>
                  </div>
                </div>
                ...
                <a
                  href="/pricing"
                  className="w-full py-3.5 px-4 rounded-2xl bg-[#FAF8F5] hover:bg-[#EEF5F3] text-[#013E37] border border-[#E2E8E5] text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-xs"
                >
                          ...
                          <td className="py-3.5 px-4 text-right">
                            {isPending && (
                              <button
                                ...
                                className="px-3 py-1.5 rounded-xl btn-primary text-xs font-bold flex items-center gap-1 ml-auto cursor-pointer"
                              >
                                <QrCode size={13} />
                            ...
                            {isPaid && (
                              <span className="text-[#267D71] font-semibold text-xs flex items-center justify-end gap-1">
